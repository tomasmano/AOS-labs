...
import cz.cvut.aos.bookingserver.dao.contract.SpecificDAO;
import cz.cvut.aos.bookingserver.dao.contract.SpecificDAORegistry;
...
public class GenericDAODispatcher implements GenericDAO {
    ...
    @Override
    public <ENTITY extends Persistable> List<ENTITY> findByProperty(String property, Object value, Class<ENTITY> clazz) {
        if (registry.containsDAO(clazz)) {
            return registry.getDAO(clazz).findByProperty(property, value);
        } else {
            return genericDAO.findByProperty(property, value, clazz);
        }
    }
